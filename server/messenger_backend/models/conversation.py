...
class Conversation(utils.CustomModel):
    ...
    def find_conversation(userIds):
        ...
        try:
            # user_ids would be a list in string format - comma-separated 
            query = reduce(operator.and_, (Q(user_ids__contains = id) for id in userIds))
            return Conversation.objects.filter(query)
